...
static README_MD: &str = include_str!("../../../static/markdown/claude-statusline-readme.md");
...
impl PageHandler for Handler {
    fn get(&self, ctx: PageContext, _state: &AppState) -> BoxFuture {
        Box::pin(async move {
            ...
            ClaudeStatuslinePage { ctx, content_html }.into_response()
        })
    }
...
#[derive(Template)]
#[template(path = "pages/misc/claude_statusline.html")]
struct ClaudeStatuslinePage {
    ctx: PageContext,
    content_html: String,
}
