...
static README_MD: &str = include_str!("../../../static/markdown/vfy-readme.md");
...
impl PageHandler for Handler {
    fn get(&self, ctx: PageContext, _state: &AppState) -> BoxFuture {
        Box::pin(async move {
            ...
            VfyBackupTesterPage { ctx, content_html }.into_response()
        })
    }
...
#[derive(Template)]
#[template(path = "pages/software/vfy_backup_tester.html")]
struct VfyBackupTesterPage {
    ctx: PageContext,
    content_html: String,
}
