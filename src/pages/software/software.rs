    ...
    fn get(&self, ctx: PageContext, state: &AppState) -> BoxFuture {
        ...
        Box::pin(async move {
            let all_pages = upvotes
                .get_all_pages(Some("defuse_software"))
                .await
                .expect("BUG: Failed to get software pages from database");
            ...
            SoftwarePage { ctx, all_pages_html }.into_response()
        })
    }
}

#[derive(Template)]
#[template(path = "pages/software/software.html")]
struct SoftwarePage {
    ctx: PageContext,
