...
static README_MD: &str = include_str!("../../../static/markdown/crackstation-hashdb-readme.md");
...
impl PageHandler for Handler {
    fn get(&self, ctx: PageContext, _state: &AppState) -> BoxFuture {
        Box::pin(async move {
            ...
            CrackstationHashdbPage { ctx, content_html }.into_response()
        })
    }
...
#[derive(Template)]
#[template(path = "pages/software/crackstation_hashdb.html")]
struct CrackstationHashdbPage {
    ctx: PageContext,
    content_html: String,
}
